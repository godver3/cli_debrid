...
class PhalanxDBClassManager:
    ...
    def __init__(self, phalanx_base_url: str = None, phalanx_port: int = None):
        ...
        # Track initialization time for startup grace period
        self._init_time = time.time()
        self._startup_grace_period = 120  # 2 minutes grace period during startup
        self._first_success = False  # Track if we've ever successfully connected
    ...
    def _try_urls(self, endpoint: str, method: str = 'GET', data: Optional[Dict] = None,
                    ...
                    try:
                        ...
                        if result:  # Found data
                            if not self._first_success:
                                ...
                                logging.info(f"PhalanxDB connection established successfully after {int(time.time() - self._init_time)}s")
        ...
        if errors and not expected_responses:
            ...
            time_since_init = time.time() - self._init_time
            in_grace_period = time_since_init < self._startup_grace_period and not self._first_success

            if in_grace_period:
                # During startup grace period, log at DEBUG level to reduce noise
                logging.debug(f"PhalanxDB not ready yet (startup grace period: {int(time_since_init)}s/{self._startup_grace_period}s) - {error_details}")
            else:
                # After grace period or after first success, log as ERROR
