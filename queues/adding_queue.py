...
class AddingQueue:
    ...
    def process(self, queue_manager: Any, ignore_upgrade_lock: bool = False) -> bool:
        ...
        # --- NZB health check: poll items already submitted to Decypharr ---
        for item in self.items[:]:
            torrent_id = item.get('filled_by_torrent_id', '')
            if not torrent_id or not str(torrent_id).startswith('nzb:'):
                continue
            item_id = item.get('id')
            ...
            try:
                ...
                try:
                    ...
                    logging.debug(f'[NZB] Error polling progress for {torrent_id}: {_pe}')
                    continue
                ...
                if progress is None:
                    ...
                    self._handle_failed_item(item, 'NZB failed in Decypharr', queue_manager)
                    continue
                elif progress < 100:
                    logging.debug(f'[NZB] {torrent_id} progress={progress}% — waiting')
                    continue

                # 100% — resolve entry name then health check
                logging.info(f'[NZB] {torrent_id} complete — running health check')
                entry_name = None
                try:
                    # Use nzb_title for fuzzy folder matching, not the UUID
                    search_name = nzb_title or job_id
                    result = client.get_nzb_file_info(search_name)
                    if result:
                        folder, _ = result
                        entry_name = folder
                except Exception as _re:
                    logging.debug(f'[NZB] Could not resolve entry name for {torrent_id}: {_re}')

                if not entry_name:
                    # Fall back to title as-is (no splitext — it's a release name not a filename)
                ...
                if not entry_name:
                    ...
                    health = client.check_entry_health(entry_name)

                if health == 'broken':
                    logging.warning(f'[NZB] {torrent_id} entry {entry_name!r} is BROKEN — deleting from Decypharr and moving back to Wanted')
                    try:
                        ...
                        # Look up real info_hash from /api/torrents by matching entry name (paginated)
                        _real_hash = None
                        ...
                        if _real_hash:
                            ...
                            logging.warning(f'[NZB] Entry {_search_name!r} not found in /api/torrents — may already be gone')
                    except Exception as _de:
                        logging.warning(f'[NZB] Error deleting broken entry from Decypharr: {_de}')
                    # Add broken NZB segment ID to not-wanted so scraper filters it across all indexers
                    try:
                        ...
                        if _seg_id:
                            ...
                            logging.info(f'[NZB] Added broken NZB segment ID {_seg_id!r} to not-wanted — blocks across all indexers')
                        else:
                            logging.debug(f'[NZB] Could not extract segment ID for {nzb_title!r}')
                    except Exception as _nwe:
                        logging.debug(f'[NZB] Could not add segment to not-wanted: {_nwe}')
                    # Clear NZB fields but keep scrape results — remove this broken result and retry
                    from database.database_writing import update_media_item as _umi
                    _umi(item_id, filled_by_torrent_id=None, filled_by_file=None, filled_by_title=None, debrid_folder_name=None, fall_back_to_single_scraper=False)
                    # Remove the broken NZB from scrape results so it won't be tried again
                    try:
                        ...
                        if _filtered and len(_filtered) < len(_results):
                            ...
                            logging.info(f'[NZB] Removed broken result from scrape list, {len(_filtered)} results remaining — retrying Adding')
                            item['scrape_results'] = _json.dumps(_filtered)
                            # Item stays in Adding — next tick processes remaining results
                        else:
                            # No scrape results stored (manually added) or all filtered — move back to Wanted for fresh scrape
                            logging.info(f'[NZB] No remaining scrape results — moving back to Wanted for fresh scrape')
                            queue_manager.move_to_wanted(item, 'Adding')
                    except Exception as _re:
                        logging.warning(f'[NZB] Could not remove broken result, moving to Wanted: {_re}')
                        queue_manager.move_to_wanted(item, 'Adding')
                else:
                    if health == 'healthy':
                    ...
                    try:
                        ...
                        self._handle_failed_item(item, f'NZB health passed but queue move failed: {_ce}', queue_manager)
            except Exception as _exc:
                logging.error(f'[NZB] Error in Adding health check for {torrent_id}: {_exc}', exc_info=True)
