...
class ScraperManager:
    ...
    def save_scrapers(self):
        try:
            ...
            config['Scrapers'] = self.scrapers
            with open(CONFIG_FILE, 'w') as config_file:
                json.dump(config, config_file, indent=2)
            logging.debug("Scrapers saved successfully")
        except Exception as e:
            logging.debug(f"Error saving scrapers: {str(e)}")
    ...
    def load_config(self):
        try:
            ...
            logging.debug(f"Error loading config: {str(e)}")
        return {}
    ...
    def add_scraper(self, scraper_type):
        ...
        self.scrapers[f"{scraper_type}_{index}"] = new_scraper
        self.save_scrapers()
        return f"{scraper_type}_{index}"

    def update_scraper(self, scraper_id, key, value):
        if scraper_id in self.scrapers:
            self.scrapers[scraper_id][key] = value
            self.save_scrapers()
            logging.debug(f"Updated {scraper_id}: {key} = {value}")
